...
suspend fun <T> PulsarTestSupport.newConsumerGroup(topic: PulsarTopic, schema: Schema<T>, consumersCount: Int, subscriptionType: SubscriptionType, subscriptionName: String = randomString(), logReceived: (ReceivedMessage<T>) -> Unit = { println("Received message $it") }): ConsumerGroup<T> {
    ...
    return ConsumerGroup(consumers, logReceived)
}